...
def render_dialogue(
    ...
    for event, item in zip(plan.events, rendered):
        if event.event_type != "barge_in" or event.timing.anchor_id is None:
            continue
        target_id = event.timing.anchor_id
        target = next((value for value in rendered if value.id == target_id), None)
        if target is None or target.speaker != "assistant":
            continue
        latency_ms = int(event.metadata.get("stop_latency_ms", 200))
        stop_ms = min(target.end_ms, item.start_ms + max(0, latency_ms))
        kept_samples = max(
            1,
            round((stop_ms - target.start_ms) * audio_by_id[target_id].sample_rate / 1000),
        )
        audio_by_id[target_id].samples = audio_by_id[target_id].samples[:kept_samples]
        target.end_ms = stop_ms
        target.duration_ms = stop_ms - target.start_ms
